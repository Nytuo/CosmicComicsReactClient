...
export default function Login() {
    ...
    const servConfig = (name: HTMLInputElement, pass: HTMLInputElement): void => {
        setOpenCreateAccount(false);
        const username = name.value.trim();
        let password = pass.value.trim();
        if (username === "") {
            ToasterHandler(t("errors.empty_fields"), "error");
            return;
        }
        if (password === "") {
            password = "_nopasswordisusedforthisaccount_";
        }
        fetch(PDP + "/configServ/" + username + "/" + password, { method: 'POST' }).then((response) => response.text()).then((data) => {
