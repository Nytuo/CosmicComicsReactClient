...
export default function MiniDrawer({
  ...
  const [createLibraryEditMode, setCreateLibraryEditMode] = React.useState<
    "add" | "edit"
  >("add");
  ...
  const [bookDiscoverProgress, setBookDiscoverProgress] = React.useState({
    x: 0,
    of: 0,
    file: "",
  });
  ...
  async function loadContent(
    ...
    await getFromDB("Series", "PATH FROM Series").then(async (res) => {
      ...
      for (const element of FolderRes) {
        const path = element;
        const name = path
          .replaceAll(libraryPath.replaceAll("\\", "/"), "")
          .replace("/", "");
        setBookDiscoverProgress({
          x: FolderRes.indexOf(element),
          of: FolderRes.length - 1,
          file: name,
        });
        console.log(name);
        let found = false;
        ...
        if (!found) {
          ...
          if (provider == providerEnum.Anilist) {
            ...
            await InsertIntoDB(
              "Series",
              "(ID_Series,title,note,statut,start_date,end_date,description,Score,genres,cover,BG,CHARACTERS,TRENDING,STAFF,SOURCE,volumes,chapters,favorite,PATH,lock)",
              "('" +
                randID +
                "U_0" +
                "','" +
      ...
      if (skip_notfound || !isOneOfThemIsNotFounded) {
        for (const element of FolderRes) {
          ...
          await getFromDB(
            "Series",
            "* FROM Series where PATH = '" + foundPATH + "'",
          ).then((resa) => {
            if (!resa) return;
            ...
            if (provider == providerEnum.Marvel) {
              ...
              node =
                JSON.parse(res[0].title)["english"] +
                " / " +
                JSON.parse(res[0].title)["romaji"] +
                " / " +
                JSON.parse(res[0].title)["native"];
            } else if (
              provider == providerEnum.MANUAL ||
              provider == providerEnum.OL ||
              provider == providerEnum.GBooks
            ...
            if (provider == providerEnum.Marvel) {
              try {
                imagelink =
                  JSON.parse(res[0].cover).path +
                  "/detail." +
                  JSON.parse(res[0].cover)["extension"];
              } catch (e) {
                imagelink = "null";
              }
            } else {
  ...
  function loadView(
    ...
    libraryPath: string,
    date: any = "",
    provider: number = providerEnum.MANUAL,
  ) {
    fetch(PDP + "/getListOfFilesAndFolders/" + FolderRes)
      .then((response) => {
      ...
      if (result) {
        ...
        DetectFolderInLibrary(result).then((data) => {
          ...
          if (dataParsed.length <= 0)
            throw new Error(t("Folderemptyornotfound"));
  ...
  React.useEffect(() => {
    if (
      openExplorer !== null &&
      openExplorer.explorer.length === openExplorer.booksNumber
    ) {
      setIsLoading(false);
    } else {
      setIsLoading(true);
  ...
  React.useEffect(() => {
    ...
    if (searchOptions.length === 0) {
      (async () => {
        await getFromDB(
          "Books",
          "NOM,PATH,URLCover,series,URLs FROM Books",
        ).then(async (resa) => {
          if (!resa) return;
          console.log(resa);
          const parsedRes = JSON.parse(resa);
          for (const element of parsedRes) {
            const provider = element.series.includes("marvel")
              ? providerEnum.Marvel
              : element.series.includes("Anilist")
                ? providerEnum.Anilist
                : element.series.includes("OL")
                  ? providerEnum.OL
                  : element.URLs.includes("google")
                    ? providerEnum.GBooks
                    : providerEnum.MANUAL;
          ...
          await getFromDB(
            "Series",
            "ID_Series,title,cover,PATH FROM Series",
          ).then(async (resaa) => {
            if (!resaa) return;
            const parsedResa = JSON.parse(resaa);
            for (const element of parsedResa) {
              const provider = element["ID_Series"].includes("_1")
                ? providerEnum.Marvel
                : element["ID_Series"].includes("_2")
                  ? providerEnum.Anilist
                  : element["ID_Series"].includes("_3")
                    ? providerEnum.OL
                    : element["ID_Series"].includes("_4")
                      ? providerEnum.GBooks
                      : providerEnum.MANUAL;
              setSearchOptions((prev) => [
                ...prev,
  ...
  return (
    <Box sx={{ display: "flex" }}>
      ...
      />
      <AddingLibraryDialog
        openModal={createLibraryOpen}
        onClose={handleCloseCreateLibrary}
        type={createLibraryEditMode}
        old={old}
      />
            ...
            <Search>
              <Autocomplete
                ...
                getOptionLabel={(option) =>
                  option.title +
                  (option.provider == providerEnum.Marvel
                    ? " | Marvel"
                    : option.provider == providerEnum.Anilist
                      ? " | Anilist"
                      : option.provider == providerEnum.OL
                        ? " | OL"
                        : option.provider == providerEnum.GBooks
                          ? " | Google Books"
                          : "")
                }
                ...
                onChange={async (_, value) => {
                  ...
                  if (value.type === "book") {
                    await getFromDB(
                      "Books",
                      "* FROM Books WHERE PATH = '" + value.path + "'",
                    ).then(async (resa) => {
                      if (!resa) return;
                    ...
                    await getFromDB(
                      "Series",
                      "* FROM Series WHERE title = '" + value.rawTitle + "'",
                    ).then(async (resa) => {
                      if (!resa) return;
                      const bookList = JSON.parse(resa);
                      ...
                      if (value.provider == providerEnum.Marvel) {
                        try {
                          imagelink =
                            JSON.parse(TheBook.cover).path +
                            "/detail." +
                            JSON.parse(TheBook.cover)["extension"];
                        } catch (e) {
                          imagelink = "null";
                        }
                      } else {
      ...
      {libraries.map((el: any, index: number) => {
        return (
          ...
          >
            ...
            <MenuItem onClick={() => handleOpenCreateLibrary("edit", el)}>
              {t("EDIT")}
            </MenuItem>
            <MenuItem onClick={() => new API().refreshMetadata(el)}>
              {t("refreshMetadata")}
            </MenuItem>
          </Menu>
        );
              ...
              >
                ...
                >
                  ...
                  >
                    <img
                      src={
                        el["API_ID"] == providerEnum.Marvel
                          ? "./Images/marvel-logo-png-10.png"
                          : el["API_ID"] == providerEnum.Anilist
                            ? "./Images/android-chrome-512x512.png"
                            : el["API_ID"] == providerEnum.MANUAL
                              ? "./Images/manual.svg"
                              : el["API_ID"] == providerEnum.OL
                                ? "./Images/OL.svg"
                                : el["API_ID"] == providerEnum.GBooks
                                  ? "./Images/Gbooks.svg"
                                  : ""
                      }
                      alt=""
