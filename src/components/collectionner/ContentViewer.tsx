...
function ContentViewer({
  ...
  const [rating, setRating] = useState<number | null>(
    TheBook.note === null ? null : parseInt(TheBook.note),
  );
    ...
    fetch(PDP + "/getListOfFilesAndFolders/" + FolderRes)
      ...
      .then(async (data) => {
        ...
        for (const element of data) {
          ...
          const readBookNB = await getFromDB(
            "Books",
            "COUNT(*) FROM Books WHERE READ = 1 AND PATH = '" + path + "'",
          );
          setReadStatSeries(
            readBookNB
  ...
  useLayoutEffect(() => {
    ...
    const fetchCreators = async () => {
      if (
        ...
        await getFromDB(
          "Creators",
          "* FROM Creators WHERE name IN (" + StaffToFetch + ")",
        ).then((clres) => {
          if (!clres) return;
          const parsedClres = tryToParse(clres);
    ...
    const fetchRelations = async () => {
      await getFromDB(
        "relations",
        "* FROM relations WHERE series = '" + TheBook.ID_book + "'",
      ).then((clres) => {
        if (!clres) return;
        const parsedClres = tryToParse(clres);
  ...
  return (
    <>
      <DatabaseEditorDialog
        openModal={openDatabaseEditorDialog}
        onClose={handleCloseDatabaseEditorDialog}
        TheBook={TheBook}
        type={type === "volume" ? "book" : "series"}
      />
      <MoreInfoDialog
        openModal={openMoreInfo}
        onClose={() => {
          closeMoreInfo();
        }}
        desc={moreInfoContent.desc}
        name={moreInfoContent.name}
        hrefURL={moreInfoContent.href}
        image={moreInfoContent.image}
        type={moreInfoContent.type}
      />
      <RematchDialog
        openModal={openRematchDialog}
        onClose={handleCloseRematchDialog}
        provider={provider}
        type={type === "volume" ? "book" : "serie"}
      ...
      >
        <img
          ...
          src="#"
          alt="#"
          style={{ width: "100vw", height: "auto", display: "none" }}
        />
        <Box
          sx={{
            width: "90vw",
          ...
          >
            ...
            >
              <img
                src={
                  type === "series"
                    ? TheBook.URLCover === null || TheBook.URLCover === "null"
                      ? "Images/fileDefault.png"
                      : TheBook.URLCover
                    : TheBook.URLCover === null || TheBook.URLCover === "null"
                      ? "Images/fileDefault.png"
                      : TheBook.URLCover.includes("public/FirstImagesOfAll")
                        ? TheBook.URLCover.split("public/")[1]
                        : TheBook.URLCover
                }
                id="ImgColCover"
                alt="#"
            ...
            >
              {type === "volume" ? (
                provider == providerEnum.Marvel ? (
                  <h1>
                    <a
                      target="_blank"
                      href={
                        TheBook.URLs === null || TheBook.URLs === "null"
                          ? "#"
                          : tryToParse(TheBook.URLs)[0].url
                      }
                    >
                      {TheBook.NOM}
                      <OpenInNew />
                ...
                <h1>
                  <a
                    target="_blank"
                    href={
                      TheBook.URLs == "null" ? "#" : tryToParse(TheBook.URLs)
                    }
                  >
                    {TheBook.NOM}
                    <OpenInNew />
              ...
              >
                ...
                {type === "series"
                  ? APINOTFOUND
                    ? tryToParse(TheBook.start_date) == null
                      ? "? -"
                      : tryToParse(TheBook.start_date) + " -"
                    : ""
                  : ""}
                {type === "series"
                  ? !APINOTFOUND
                    ? (provider == providerEnum.Marvel
                      ...
                      : provider == providerEnum.Marvel
                        ? " " + tryToParse(TheBook.end_date)
                        : " " + tryToParse(TheBook.end_date).year
                    : " ?"
                  : ""}
                {type === "series"
                  ? APINOTFOUND
                    ? tryToParse(TheBook.end_date) == null ||
                      tryToParse(TheBook.end_date) > new Date().getFullYear()
                      ? " ?"
                      : " " + tryToParse(TheBook.end_date)
                    : ""
              ...
              <Stack spacing={3}>
                {" "}
                <Grid2 container spacing={2} id="btnsActions">
                    ...
                    <IconButton
                      ...
                      onClick={async () => {
                        if (type == "volume") {
                          ...
                          await getFromDB(
                            "Books",
                            "PATH FROM Books WHERE unread=1 OR reading=1",
                          ).then(async (resa) => {
                            if (!resa) return;
                            ...
                            for (const element of bookList) {
                              if (
                                element.PATH.toLowerCase().includes(
                                  resolveTitle(TheBook.raw_title)
                                    .toLowerCase()
                                    .replaceAll('"', ""),
                                )
                              ) {
                  ...
                  <Tooltip title={t("rematch")}>
                    <IconButton
                      id="rematchBtn"
                      onClick={() => handleOpenRematchDialog()}
                    >
                      {" "}
                      <YoutubeSearchedFor />
                    </IconButton>
                ...
                TheBook.characters !== "null" && TheBook.characters !== "NULL" ? (
                  <div id="readstat">
                    <input
                      ...
                      min="0"
                      id="readAddInput"
                      value={TheBook.last_page}
                      max={TheBook.pageCount}
                      onBlur={async (e) => {
                        const options = {
                          method: "POST",
                          headers: {
                        ...
                        await fetch(PDP + "/DB/update", options).catch(
                          (err) => {
            ...
            >
              ...
              <div style={{ textAlign: "center" }}>
                <p id="provider_text">
                  {provider == providerEnum.Marvel
                    ? t("providedBy") + " Marvel. © 2014 Marvel"
                    : provider == providerEnum.Anilist
                      ? t("providedBy") + " Anilist."
                      : provider == providerEnum.MANUAL
                        ? t("notFromAPI")
                        : provider == providerEnum.OL
                          ? t("providedBy") + " OpenLibrary."
                          : provider == providerEnum.GBooks
                            ? t("providedBy") + " Google Books."
                            : t("notFromAPI")}
                </p>
              </div>
            </Box>
