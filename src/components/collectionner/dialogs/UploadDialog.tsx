    ...
    xhr.onreadystatechange = function () {
      if (xhr.readyState === 4 && xhr.status === 200) {
        ...
        if (response === "Upload successful") {
          const fileUPElement = document.getElementById(
            "fileUp",
          ) as HTMLInputElement;
          if (!fileUPElement) return;
          if (!fileUPElement.files) return;
          const url =
            cosmicComicsTemp + "/uploads/" + fileUPElement.files[0].name;
        ...
        <DialogContent>
          <DialogContentText>
            ...
            >
              ...
              <Button style={{ marginTop: "10px" }} type="reset" id="resetBtn">
                {t("reset")}
              </Button>
            </form>
          </DialogContentText>
